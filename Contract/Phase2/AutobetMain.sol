...
import "https://github.com/smartcontractkit/chainlink/blob/0.8.4/evm-contracts/src/v0.6/VRFConsumerBase.sol";
...
contract Autobet is VRFConsumerBase{
    ...
    function Numbers(uint lotteryid) public view returns (int[] memory tickets,address[] memory useraddress)  {
        LotteryData storage LotteryDatas = lottery[lotteryid];
        uint k = 0;
        uint p=0;
    ...
    function getdraw(uint256 num,uint lotteryid)public {
        LotteryData storage LotteryDatas = lottery[lotteryid];
        uint256 numbers = uint256(keccak256(abi.encodePacked(num,uint256(0),now)));
        numbers =numbers % LotteryDatas.Tickets.length+1;
        LotteryDatas.status =LotteryState.resultdone;
        LotteryDatas.lotteryWinner=LotteryDatas.Tickets[numbers].user;
        emit lotteryresuls(LotteryDatas.Tickets[numbers].user,lotteryid,now);
        paywinner(                                                         );
    }
    ...
    function paywinner( address useraddressdata , uint lotteryid) public payable{
        LotteryData storage LotteryDatas = lottery[lotteryid];
        uint totalPrize = LotteryDatas.totalPrize;
        if(useraddressdata!= address(0)){
            address(uint160(useraddressdata)).transfer(totalPrize);
            emit prizedistributed(useraddressdata,totalPrize,lotteryid,now);
        }
        callresult = true;
    }
